...
{
  ASSERT_THROW (Game (TITLE, GetParam(), DEFAULT_HEIGHT, INIT_FLAGS, WINDOW_FLAGS,
                      font_path, DEFAULT_TPS, DEFAULT_FPS),
                std::invalid_argument);
}

TEST_P (GameConstructorTests, InvalidHeight) {
  ASSERT_THROW (Game (TITLE, DEFAULT_WIDTH,GetParam(), INIT_FLAGS, WINDOW_FLAGS,
                      font_path, DEFAULT_TPS, DEFAULT_FPS),
                std::invalid_argument);
}

TEST_P (GameConstructorTests, InvalidTPS) {
  ASSERT_THROW(Game (TITLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, INIT_FLAGS, WINDOW_FLAGS,
                      font_path, GetParam(), DEFAULT_FPS), std::invalid_argument);
}

TEST_P (GameConstructorTests, InvalidFPS) {
  ASSERT_THROW(Game (TITLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, INIT_FLAGS, WINDOW_FLAGS,
                      font_path, DEFAULT_TPS, GetParam()), std::invalid_argument);
}

INSTANTIATE_TEST_SUITE_P(InvalidIntArgs, GameConstructorTests,testing::Values(0,-1,MIN)); 

TEST_F(GameConstructorTests, InvalidFontPath) {
  ASSERT_THROW(Game (TITLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, INIT_FLAGS, WINDOW_FLAGS,
                     "", DEFAULT_TPS, DEFAULT_FPS), std::runtime_error);
}
TEST_F(GameConstructorTests, ValidConstructor) {
  ASSERT_NO_THROW(Game (TITLE, DEFAULT_WIDTH, DEFAULT_HEIGHT, INIT_FLAGS, WINDOW_FLAGS,
                     font_path,DEFAULT_TPS, DEFAULT_FPS));
}
