...
TEST(ArithmeticTests, IllegalFirstOperand) {
  ...
  ASSERT_THROW(calculate_dimension(std::numeric_limits<int>::min(), 0), std::logic_error);
}

TEST(ArithmeticTests, IllegalSecondOperand) {
  ASSERT_THROW(calculate_dimension(std::numeric_limits<int>::max(), 0), std::logic_error);
  ASSERT_THROW(calculate_dimension(std::numeric_limits<int>::max(), -1), std::logic_error);
  ASSERT_THROW(calculate_dimension(1, std::numeric_limits<int>::min()), std::logic_error);
}
